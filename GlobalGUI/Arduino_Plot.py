...
def Init_Arduino(self):
    ...
    def Take_Info():
        ...
        self.number_of_samples = float(self.ui.load_pages.textEdit.toPlainText())
        ...
        self.CounterRMS=0
        self.CounterRMS2=0
        # Change Arduino intensity
        self.threadArduino.intensity=self.intensity
        # Creation of the new DAQ thread
        data_obj = td.DAQData(self.Number_to_mean, self.Laser_frequency, int(self.number_of_samples), self.DAQ_Device, self.lowFreq, self.highFreq, int(self.fileSave),self.DAQ_Device2)
        self.threadDAQ=data_obj
        # Start the plot processing
        self.timerPLOT.start()
        # Resize of axes in FFT
        self.Freqx = list(range(int(self.number_of_samples)))  
        self.Freqy = [randint(0,100) for _ in range(int(self.number_of_samples))]  
